...
- aoi can be in any of the 20 most comfortable (qwerty) keys: "FDSVERAWCQJKLMIU;O,P"
- positions_to_assign: 15 of the remaining 20 keys

Step 1a: positions_assigned
---------------------------
e in (qwerty) key D/F, t in key J/K:
╭───────────────────────────────────────────────╮
│     │     │     │     ║     │     │     │     │
├─────┼─────┼─────┼─────╫─────┼─────┼─────┼─────┤
│     │     │  D  │  F  ║  J  │  K  │     │     │
...
---------------------------
aoi in any of the top-20 (most comfortable) keys, 
...
There are 19,584 valid configurations based on the constraints.

Example configuration:
  items_assigned: etaoi
  positions_assigned: DJFSV
  positions_to_assign: ERAWCQKLMIU;O,P
  Letter mappings:
    e -> D
    t -> J
    a -> F
...
if __name__ == "__main__":
    nlayouts = 100
    print("Generating {0} keyboard layout configurations...".format(nlayouts))
    ...
    aoi_positions = top_keys - 2  # Excluding where e and t are placed
    aoi_combinations = len(list(itertools.combinations(range(aoi_positions), 3)))
    aoi_permutations = 6  # 3! = 6 ways to arrange a,o,i
    
    print(f"\nTheoretical maximum configurations: {e_positions * t_positions * aoi_combinations * aoi_permutations}")
